...
from fairnr.modules.discriminator import get_discriminator
...
class BaseModel(nn.Module):
    ...
    DISCRIMINATOR = 'abstract_discriminator'

    def __init__(self, args, reader, encoder, field, raymarcher, discriminator=None):
        super().__init__()
        self.args = args
        self.reader = reader
    ...
    def build_model(cls, args):
        ...
        if args.dis:
            discriminator = get_discriminator(cls.DISCRIMINATOR)(args)
            return cls(args, reader, encoder, field, raymarcher, discriminator)
        else:
    ...
    def add_args(cls, parser):
        ...
        get_discriminator(cls.DISCRIMINATOR).add_args(parser)
    ...
    def forward(self, ray_split=1, **kwargs):
        ...
        ray_start, ray_dir, uv, dis_ray_start, dis_ray_dir = self.reader(**kwargs)
        kwargs.update({
            'field_fn': self.field.forward,
            'input_fn': self.encoder.forward})

        if ray_split == 1:
            results = self._forward(ray_start, ray_dir, dis_ray_start, dis_ray_dir, **kwargs)
        else:
            total_rays = ray_dir.shape[2]
            chunk_size = total_rays // ray_split
            results = [
                self._forward(
                    ray_start, ray_dir[:, :, i: i+chunk_size], dis_ray_start, dis_ray_dir[:, :, i: i+chunk_size], **kwargs)
    ...
    @torch.no_grad()
    def visualize(self, sample, output=None, shape=0, view=0, raw_depth=False, **kwargs):
        ...
        if raw_depth:
            for tag in images:
                images[tag] = recover_image(width=width, **images[tag]) if (images[tag] is not None and tag.split('/')[0].split('_')[1] != 'voxeldepth') else images[tag]['img']
        else:
            images = {
    ...
    def _visualize(self, images, sample, output, state, **kwargs):
        ...
        if 'depths' in output and output['depths'] is not None and 'ray_start' in output:
        ...
        if 'voxel_depth' in output and output['voxel_depth'] is not None:
            min_depth, max_depth = output['voxel_depth'].min(), output['voxel_depth'].max()
            images['{}_voxeldepth/{}:HWC'.format(name, img_id)] = {
                'img': output['voxel_depth'][shape, view], 
                'min_val': min_depth, 
                'max_val': max_depth}
