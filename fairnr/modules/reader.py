...
from fairnr.data.geometry import get_ray_direction, r6d2mat, get_ray_direction_proj
...
class ImageReader(Reader):
    ...
    def __init__(self, args):
        ...
        self.num_pixels_ratio = getattr(args, "pixel_per_view_down", None)
        if self.num_pixels_ratio is not None:
            self.num_pixels_ratio = [float(s) for s in self.num_pixels_ratio.split(',')]
    ...
    def add_args(parser):
        ...
        parser.add_argument('--pixel-per-view-down', type=str, 
                            help='ratio of downsample pixels sampled for each view')
    ...
    def forward(self, uv, intrinsics, extrinsics, size, projections=None, path=None, **kwargs):
        S, V = uv.size()[:2]
        ...
        if self.args.dis and 'dis_extrinsics' in kwargs:
            dis_ray_start, dis_ray_dir = [[] for _ in range(S)], [[] for _ in range(S)]
        for s in range(S):
            for v in range(V):
                ...
                if projections is not None:
                    proj = projections[s, v]
                    ray_dir[s] += [get_ray_direction_proj(translation, flatten_uv[s, v], ext, proj, size, 1)]
                else:
                    ray_dir[s] += [get_ray_direction(translation, flatten_uv[s, v], ixt, ext, 1)]

                if self.args.dis and 'dis_extrinsics' in kwargs:
                    dis_ext = kwargs['dis_extrinsics'][s, v]
                    dis_translation, dis_rotation = dis_ext[:3, 3], dis_ext[:3, :3]
                    dis_ray_start[s] += [dis_translation]
                    if projections is not None:
                        dis_proj = kwargs['dis_projections'][s, v]
                        dis_ray_dir[s] += [get_ray_direction_proj(dis_translation, flatten_uv[s, v], dis_ext, dis_proj, size, 1)]
                    else:
                        dis_ray_dir[s] += [get_ray_direction(dis_translation, flatten_uv[s, v], ixt, dis_ext, 1)]
                    
        ray_start = torch.stack([torch.stack(r) for r in ray_start])
        ray_dir = torch.stack([torch.stack(r) for r in ray_dir])
        if self.args.dis and 'dis_extrinsics' in kwargs:
            dis_ray_start = torch.stack([torch.stack(r) for r in dis_ray_start])
            dis_ray_dir = torch.stack([torch.stack(r) for r in dis_ray_dir])
            return ray_start.unsqueeze(-2), ray_dir.transpose(2, 3), uv, dis_ray_start.unsqueeze(-2), dis_ray_dir.transpose(2, 3)
        else:
            return ray_start.unsqueeze(-2), ray_dir.transpose(2, 3), uv, None, None
    ...
    @torch.no_grad()
    def downsample_pixels_num(self, times):
        if self.num_pixels_ratio is not None:
            self.num_pixels = int(self.num_pixels * self.num_pixels_ratio[times])
