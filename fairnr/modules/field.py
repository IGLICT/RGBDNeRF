...
class RaidanceField(Field):
    ...
    def __init__(self, args):
        ...
        if self.load_pc:
            self.color_func = NeRFPosEmbLinear(
                3, 3 * self.args.pc_pose_dim * 2, 
                angular=False, no_linear=True, cat_input=True)
            render_input_dim = tex_input_dim + 3 + self.color_func.out_dim
        else:
            render_input_dim = tex_input_dim
        self.predictor = SignedDistanceField(den_feat_dim, args.density_embed_dim, recurrent=False)
        self.renderer = TextureField(render_input_dim, args.texture_embed_dim, args.texture_layers) 
    ...
    def forward(self, inputs, outputs=['sigma', 'texture'], color_feat=None):
        ...
        if color_feat is not None:
            inputs['color_feat'] = self.color_func(color_feat)
        ...
        if 'texture' in outputs:        
            ...
            filtered_inputs = torch.cat(filtered_inputs, -1)
            if self.load_pc and 'color_feat' in inputs:
                filtered_inputs = torch.cat((filtered_inputs, inputs['color_feat']), dim=-1)
