...
def transfer_uv(uv, size):
    h, w = int(size[0,0,0]*size[0,0,2]), int(size[0,0,1]*size[0,0,3])
    h_rate = (2. / h)
    w_rate = (2. / w)
    new_uv = uv.clone()
    new_uv[0] = w_rate * uv[0] - 1 + w_rate / 2  # Image 0 -> Coor2d -1 + 1/w, Image w-1 -> Coor2d 1 - 1/w
    new_uv[1] = h_rate * (h-1-uv[1]) - 1 + h_rate / 2  # Image 0 -> Coor2d -1 + 1/h, Image h-1 -> Coor2d 1 - 1/h
    return new_uv

def homogeneous(coor):
    if isinstance(coor, torch.Tensor):
        coor = torch.cat([coor, torch.ones([coor.shape[0], 1]).type_as(coor)], dim=1)
    else:
        coor = np.concatenate([coor, np.ones([coor.shape[0], 1])], 1)
    return coor

def uv2world_proj(projectionMatrix, cameraPose, xy, depth):

    xy = xy.T
    if isinstance(depth, torch.Tensor):
        depth = depth.T / 1000
        coor_cvv = torch.cat([xy, depth], dim=1) 
    else:
        coor_cvv = torch.cat([xy, torch.ones([xy.shape[0], 1]).type_as(xy)*depth], dim=1)
    point_num = coor_cvv.shape[0]
    p01 = projectionMatrix[:2].unsqueeze(0).expand([point_num, 2, 4])
    p3 = projectionMatrix[-1:].unsqueeze(0).expand([point_num, 1, 4])
    cvv_xy = coor_cvv[:, :2].unsqueeze(-1).expand([point_num, 2, 1])
    M = p01 - torch.einsum('nto,nof->ntf', cvv_xy, p3)
    z = torch.stack([- coor_cvv[:, 2], torch.ones_like(coor_cvv[:, 2])], dim=-1).unsqueeze(-1)
    b = torch.einsum('nst,ntk->nsk', M[:, :, 2:], z)
    camera_xy = - torch.einsum('nts,nsk->ntk', torch.inverse(M[:, :, :2]), b).squeeze(-1)
    coor_camera = torch.cat([camera_xy, - coor_cvv[:, 2].unsqueeze(-1)], dim=-1)

    coor_camera = homogeneous(coor_camera)
    # cameraPose[:,1] *= -1
    # cameraPose[:,2] *= -1
    coor3d = torch.mm(cameraPose, coor_camera.T).T

    return coor3d[:, :3].T

def get_ray_direction_proj(ray_start, uv, inv_RT, proj, size, depths=None):
    if depths is None:
        depths = 1
    new_uv = transfer_uv(uv, size)
    rt = uv2world_proj(proj, inv_RT, new_uv, depths)
    ray_dir, _ = normalize(rt - ray_start[:, None], axis=0)
    return ray_dir
